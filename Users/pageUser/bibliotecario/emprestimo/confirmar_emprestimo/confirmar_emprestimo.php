...
try {
    // --- CORREÇÃO: Determinar o n_emprestimo ANTES de iniciar a transação ---
    // Pega o maior n_emprestimo existente e soma 1.
    // CUIDADO: Ainda pode ter problemas de concorrência, mas é melhor que antes.
    $sql_get_next_n = "SELECT MAX(n_emprestimo) as max_n FROM tbemprestimos";
    $result_n = $conn->query($sql_get_next_n);
    if (!$result_n) {
        // Não iniciar transação se não conseguir determinar o próximo número
        throw new Exception(
    ...
    $numero_emprestimo_grupo = intval($row_n["max_n"] ?? 0) + 1;
    $result_n->free();
    ...
    foreach ($livros as $index => $livro) {
        ...
        if (empty($livro["tombo"])) {
            throw new Exception(
                "Livro '{$livro["titulo"]}' sem TOMBO definido na seleção."
            );
        }
    ...
    error_log(
        "ERRO FINALIZAR EMPRESTIMO (Reestruturado V2): " . $e->getMessage()
    );
    echo json_encode([
        "success" => false,
        "error" =>

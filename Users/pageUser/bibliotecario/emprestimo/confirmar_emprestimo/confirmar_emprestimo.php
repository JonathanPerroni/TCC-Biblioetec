...
document.getElementById('btn-confirmar').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch('finalizar_emprestimo.php', {
            ...
            // Não precisa enviar body, pois os dados estão na sessão
        ...
        try {
            ...
            console.error('Resposta recebida:', text); // Log para debug
            throw new Error('Resposta do servidor não é JSON válido.');
        }

        if (!response.ok) {
            throw new Error(data.error || 'Erro na requisição ao finalizar empréstimo.');
        }

        // *** CORREÇÃO APLICADA AQUI ***
        if (data.success && data.n_emprestimo) { 
            // Redireciona usando n_emprestimo
            window.location.href = 'comprovante.php?n_emprestimo=' + data.n_emprestimo; 
        } else {
            // Lida com erro ou falta do n_emprestimo na resposta
            throw new Error(data.error || 'Erro ao finalizar empréstimo ou número do grupo não retornado.');
        }
    } catch (error) {
        console.error('Erro ao confirmar empréstimo:', error);
        alert('Erro ao confirmar empréstimo: ' + error.message);

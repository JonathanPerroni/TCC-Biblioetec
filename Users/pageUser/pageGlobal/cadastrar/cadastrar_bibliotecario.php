<?php
 ...
 if (empty($_SESSION['msg'])) {
     $query_verifica_cpf = "SELECT COUNT(*) AS total FROM tbbibliotecario WHERE cpf = ?";
     ...
     if ($row_verifica_cpf['total'] > 0) {
         $_SESSION['msg'] .= "CPF já cadastrado!";
             ...
             if (empty($errors['email']) && empty($errors['confirma_email'])) {
                 $query_verifica_email = "SELECT COUNT(*) AS total FROM tbbibliotecario WHERE email = ?";
...
<head>
    ...
    <title>Cadastro do Bibliotecario</title>
...
<body class="w-100 h-auto d-flex flex-column align-itens-center">
        ...
        <a href="<?php echo $pagina_inicial; ?>" class="d-flex align-items-center position-absolute start-0 ms-4 nav-link">
            ...
            <a href="#" class="nav-link fs-3 fw-medium text-primary">Cadastrar Bibliotecario</a>
        </header>

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sqlSelect = "SELECT nome, acesso FROM tbbibliotecario WHERE codigo = ?";
    ...
    if (!$nome_usuario) {
        echo "<script>alert('Bibliotecario não encontrado!');</script>";
        exit;
    }

    // Define a consulta para excluir o registro
    $sqlDelete = "DELETE FROM tbbibliotecario WHERE codigo = ?";
    ...
    if ($stmtDelete->execute()) {
        ...
        echo "<script>
            alert('O Bibliotecario  \"$nome_usuario\" foi excluído com sucesso!');
            window.location.href = '../list/listabibliotecarioNew.php';
        </script>";
    } else {

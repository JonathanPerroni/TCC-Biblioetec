...
$idade_minima = $conn->real_escape_string($idade_minima);
$num_jogadores = $conn->real_escape_string($num_jogadores);
$fabricante = $conn->real_escape_string($fabricante);
...
try {
    ...
    $sql = "UPDATE tbjogoseducativos SET 
                classe = ?, 
                titulo = ?, 
                categoria = ?, 
                idade_minima = ?, 
    ...
    if ($stmt->execute()) {
        ...
        header("Location: editarJogos.php?codigo=" . urlencode($codigo));
        exit;
    } else {

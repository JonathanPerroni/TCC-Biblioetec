...
if ($cpfErro !== true) {
    ...
    $queryCpf = "SELECT COUNT(*) FROM tbbibliotecario WHERE cpf = ? AND codigo != ?";
...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    $queryEmail = "SELECT COUNT(*) FROM tbbibliotecario WHERE email = ? AND codigo != ?";
...
if (!empty($_SESSION['msg'])) {
    header("Location: editarBibliotecario.php?codigo=" . urlencode($codigo));
    exit();
}
...
try {
    ...
    $sql = "UPDATE tbbibliotecario SET
                nome = ?, 
                email = ?, 
                password = ?, 
                telefone = ?, 
    ...
    $stmt->bind_param("sssssssssi", $nome, $email, $hashedPassword, $telefone, $celular, $cpf, $codigo_escola, $acesso, $status, $codigo);

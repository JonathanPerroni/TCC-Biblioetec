...
// Escapa o valor da pesquisa para evitar injeção de SQL
$pesquisa_escapada = mysqli_real_escape_string($conn, $pesquisa);

// Define o charset do banco de dados como UTF-8
...
<head>
    ...
    <title>BiblioEtec - Acervos Cadastrados</title>
...
<body class="w-screen h-screen flex flex-col items-center bg-[var(--off-white)]">
    ...
    <main class="mt-4 w-full flex flex-col items-center gap-4">
        <form name="pesquisa" action="pagealuno.php" method="get" class="w-full md:w-[75%] flex flex-col md:flex-row items-center justify-between gap-2 px-2 py-2 bg-white rounded-md shadow">
            <label for="pesquise" class="font-medium text-secondary">Pesquisar Livro:</label>
            <input type="text" name="pesquisa" value="<?php echo htmlspecialchars($pesquisa); ?>" class="w-full border border-secondary rounded text-secondary px-2 py-1">
            <input type="submit" value="Pesquisar" class="bg-secondary text-white rounded shadow px-4">
        </form>
